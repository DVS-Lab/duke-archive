...
template_f = file("framing_task_m4.sh")  #job template location (on head node)
...
sleep_time = 3              #pause time (sec) between job count checks
...
#subnums = ["48335"] #just a quick re-do from above
...
for go in goes:
	for subnum in subnums:
		...
		for run in runs:
			...
			tmp_job_fname = "_".join( ["L1_m4", subnum, runstr, go] )
