...
FSLEVDIR=$EXPERIMENT/Analysis/Framing/EVfiles2/model5/${SUBJ}
ANAT=$EXPERIMENT/Analysis/TaskData/${SUBJ}/${SUBJ}_anat_brain.nii.gz
MAINOUTPUT=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}
OUTDIR=$EXPERIMENT/Analysis/Framing/Logs/DVS/L1_m5/$SUBJ
mkdir -p $OUTDIR
...
EV5=${FSLEVDIR}/run${run}/${SUBJ}_cue_run${run}.txt

OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model5_run${run}
...
TEMPLATE=$EXPERIMENT/Analysis/Framing/Templates/L1_m5_template.fsf
sed -e 's@DATA@'$DATA'@g' \
-e 's@ANAT@'$ANAT'@g' \
-e 's@OUTPUT@'$OUTPUT'@g' \
-e 's@EV1@'$EV1'@g' \
...
<$TEMPLATE> ${MAINOUTPUT}/run${run}/${SUBJ}_model5_run${run}.fsf

#run the newly created fsf files
if [ -d $OUTPUT.feat ]; then
	echo "$OUTPUT.feat exists! skipping to the next one"
else
	$FSLDIR/bin/feat ${MAINOUTPUT}/run${run}/${SUBJ}_model5_run${run}.fsf
fi
