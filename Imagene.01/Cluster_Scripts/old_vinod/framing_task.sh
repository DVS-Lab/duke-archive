...
OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model1c_run${run}
...
TEMPLATE=$EXPERIMENT/Analysis/Framing/Templates/template_model1_new.fsf
sed -e 's@DATA@'$DATA'@g' \
-e 's@ANAT@'$ANAT'@g' \
-e 's@OUTPUT@'$OUTPUT'@g' \
-e 's@EV1@'$EV1'@g' \
...
-e 's@EV6@'$EV6'@g' \
<$TEMPLATE> ${MAINOUTPUT}/run${run}/${SUBJ}_model1c_run${run}.fsf
...
$FSLDIR/bin/feat ${MAINOUTPUT}/run${run}/${SUBJ}_model1c_run${run}.fsf
