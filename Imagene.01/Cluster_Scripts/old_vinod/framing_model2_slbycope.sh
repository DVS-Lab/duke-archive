...
OUTPUT=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}/${SUBJ}_model3_slbycope
OUTFILE=$OUTPUT/cope${cnum}
COPE1=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}/run1/${SUBJ}_model3_run1.feat/stats/cope${cnum}.nii.gz
COPE2=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}/run2/${SUBJ}_model3_run2.feat/stats/cope${cnum}.nii.gz
COPE3=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}/run3/${SUBJ}_model3_run3.feat/stats/cope${cnum}.nii.gz
OUTDIR=$EXPERIMENT/Analysis/Framing/Logs
 
for i in $EXPERIMENT/Analysis/Framing/Templates/'template_model3_slbycope.fsf'; do
sed -e 's@OUTPUT@'$OUTFILE'@g' \
    -e 's@COPE1@'$COPE1'@g' \
    -e 's@COPE2@'$COPE2'@g' \
   -e 's@COPE3@'$COPE3'@g' <$i> ${OUTFILE}.fsf
done
