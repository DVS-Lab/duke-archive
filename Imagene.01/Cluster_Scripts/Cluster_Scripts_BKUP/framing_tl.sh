...
EXPTDIR=$EXPERIMENT/Analysis/Framing/FSL
CNUM=$1

for i in $EXPERIMENT/Analysis/Framing/Templates/'N35_model3b_tlcov.fsf'; do
sed -e 's@EXPTDIR@'$EXPTDIR'@g' \
   -e 's@CNUM@'$CNUM'@g' <$i> ${EXPTDIR}/Level3/N35_Model3b/N35_model3b_cov_cope${CNUM}.fsf
done
 
#run the newly created fsf files
feat ${EXPTDIR}/Level3/N35_Model3b/N35_model3b_cov_cope${CNUM}.fsf

#data not hand transfered on cluster
#cleans up golgi output and transfers analysis
#cp -r ${MAINOUTPUT} ${ANALYZED}
#rm -r ${MAINOUTPUT}
