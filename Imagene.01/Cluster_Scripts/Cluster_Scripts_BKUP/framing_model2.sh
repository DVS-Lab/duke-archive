...
OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model3b_run${run}
DATA=${FSLDATADIR}/run${run}.feat/new_filtered_func_data.nii.gz
 
 
for i in $EXPERIMENT/Analysis/Framing/Templates/'template_model3.fsf'; do
sed -e 's@DATA@'$DATA'@g' \
    -e 's@ANAT@'$ANAT'@g' \
    -e 's@OUTPUT@'$OUTPUT'@g' \
    -e 's@EV1@'$EV1'@g' \
    ...
    -e 's@EV8@'$EV8'@g' \
    -e 's@EV9@'$EV9'@g' \
   -e 's@EV0@'$EV0'@g' <$i> ${MAINOUTPUT}/run${run}/${SUBJ}_model3b_run${run}.fsf
done
...
feat ${MAINOUTPUT}/run${run}/${SUBJ}_model3b_run${run}.fsf
