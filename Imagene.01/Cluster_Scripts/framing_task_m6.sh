...
FSLEVDIR=$EXPERIMENT/Analysis/Framing/EVfiles2/model6/${SUBJ}
ANAT=$EXPERIMENT/Analysis/TaskData/${SUBJ}/${SUBJ}_anat_brain.nii.gz
MAINOUTPUT=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}
OUTDIR=$EXPERIMENT/Analysis/Framing/Logs/DVS/L1_m6/$SUBJ
mkdir -p $OUTDIR
...
OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model6_run${run}
...
<$TEMPLATE> ${MAINOUTPUT}/run${run}/${SUBJ}_model6_run${run}.fsf

#run the newly created fsf files
if [ -d $OUTPUT.feat ]; then
	echo "$OUTPUT.feat exists! skipping to the next one"
else
	$FSLDIR/bin/feat ${MAINOUTPUT}/run${run}/${SUBJ}_model6_run${run}.fsf
fi
