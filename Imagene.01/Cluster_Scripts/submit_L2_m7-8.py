...
template_f = file("framing_task_L2_m7-8.sh")  #job template location (on head node)
experiment = "Imagene.01"    #experiment name for qsub
nodes = 24                   #number of nodes on cluster
maintain_n_jobs = 23         #leave one in q to keep them moving through
...
for go in goes:
	for model in models:
		...
		for subnum in subnums:
			...
			tmp_job_fname = "_".join( ["L2_m", model, subnum, go ] )
