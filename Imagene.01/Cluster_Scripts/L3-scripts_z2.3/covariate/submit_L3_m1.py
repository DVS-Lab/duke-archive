...
template_f = file("L3_m1_n58.sh")  #job template location (on head node)
...
min_jobs = 4                 #minimum number of jobs to keep in q even when crowded
...
max_run_time = 200           #maximum time any job is allowed to run in minutes
...
copes = range(1,21)
cnames = ["charcue", "selfcue", "chargain", "charloss", "selfgain", "selfloss", "charcue-selfcue", "selfcue-charcue", "gain-loss", "loss-gain", "char-self", "self-char", "chargain-charloss", "charloss-chargain", "selfgain-selfloss", "selfloss-selfgain", "chargain-selfgain", "selfgain-chargain", "charloss-selfloss", "selfloss-charloss"]

covariates = ["L3_n58_charity_cov", "L3_n58_self_cov", "L3_n58_S-C_cov"]
...
for covariate in covariates:
	...
	for cope in copes:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_COV_SUB", str(covariate) )

	
		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["L3m1", cname, str(cope), covariate ] )
