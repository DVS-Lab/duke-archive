...
template_f = file("L1_framing_ALL_PPI.sh")  #job template location (on head node)
...
min_jobs = 14                 #minimum number of jobs to keep in q even when crowded
...
max_run_time = 99999999           #maximum time any job is allowed to run in minutes
max_run_hours = 99999999	#maximum number of hours submission script can run
warning_time = 99999999         #send out a warning after this many hours informing you that the deamon is still running
...
goes = [ "17" ]
models = [ "10" ]
#rois = [ "dmPFC_5mm", "aVMPFC_5mm", "pVMPFC_5mm", "LR_NAcc", "R_NAcc", "L_NAcc", "LR_Amyg", "R_Amyg", "L_Amyg" ]
#rois = [ "pSTC_5mm", "midTG_5mm", "IFG_5mm" ]


rois = [ "L_FPole_func_5mm", "L_iFG_func_5mm", "L_lOFC_func_5mm" ]


#rois = [ "dmPFC", "aVMPFC", "pVMPFC" ]
contrasts = [ "char-self", "gain-loss", "interaction" ]
#EV01=${FSLEVDIR}/run${run}/${SUBJ}_char-self_run${run}.txt
#EV01=${FSLEVDIR}/run${run}/${SUBJ}_charpar-selfpar_run${run}.txt
#EV01=${FSLEVDIR}/run${run}/${SUBJ}_gain-loss_run${run}.txt
#EV01=${FSLEVDIR}/run${run}/${SUBJ}_gainpar-losspar_run${run}.txt
#EV01=${FSLEVDIR}/run${run}/${SUBJ}_interaction_run${run}.txt
#EV01=${FSLEVDIR}/run${run}/${SUBJ}_interaction_par_run${run}.txt
			...
			for roi in rois:
				...
				for subnum in subnums:
					...
					for run in runs:
						...
						tmp_job_file = tmp_job_file.replace( "SUB_ROI_SUB", str(roi) )
						tmp_job_file = tmp_job_file.replace( "SUB_CON_SUB", str(contrast) )
				
						#make fname and write job file to cwd
						tmp_job_fname = "_".join( ["ppi_L1m", model, subnum, runstr, roi, contrast, go] )
