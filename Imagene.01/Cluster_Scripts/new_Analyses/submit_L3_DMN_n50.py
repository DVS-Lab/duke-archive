...
template_f = file("L3_DMN_n50.sh")  #job template location (on head node)
experiment = "Imagene.01"    #experiment name for qsub
nodes = 28                   #number of nodes on cluster
maintain_n_jobs = 28         #leave one in q to keep them moving through
...
warning_time = 180         #send out a warning after this many hours informing you that the deamon is still running

#make job files  these are the lists to be traversed
#all iterated items must be in "[ ]" separated by commas.  
copes = range(1,21)

cnames = [ "chargain", "charloss", "selfgain", "selfloss", "motor", "cue", "chargain-charloss", "charloss-chargain", "selfgain-selfloss", "selfloss-selfgain", "self-char", "char-self", "gain-loss", "loss-gain", "chargain-selfgain", "selfgain-chargain", "charloss-selfloss", "selfloss-charloss", "interaction+", "interaction-" ]

fsftemplates = [ "PCC3_MPFC3", "PCC1_MPFC1", "PCC1_MPFC2", "PCC1_MPFC3", "PCC1_MPFC4", "PCC2_MPFC1", "PCC2_MPFC2", "PCC2_MPFC3", "PCC2_MPFC4", "PCC3_MPFC1", "PCC3_MPFC2", "PCC3_MPFC4" ] #making PCC3_MPFC3 first because it had the most connectivity across subjects
	...
	for fsftemplate in fsftemplates:
		for model in models:
			for cope in copes:
				...
				tmp_job_file = tmp_job_file.replace( "SUB_ISPARAMETRIC_SUB", str(ispara) )
	
				#make fname and write job file to cwd
				tmp_job_fname = "_".join( [ "DMN_m", str(model), cname, str(cope), fsftemplate , ispara] )
