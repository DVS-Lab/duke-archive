...
template_f = file("L3_ALL_n58_PPI.sh")  #job template location (on head node)
...
max_run_hours = 9999999	#maximum number of hours submission script can run
...
fsftemplates = [ "L3_n58", "L3_n58_cov"]
...
#"charpar-selfpar", "gainpar-losspar", "interaction_par"
			...
			for roi in rois:
				for model in models:
					for cope in copes:
						...
						cname = cnames[cope-3] #this is a cheap hack to make it work.
						...
						tmp_job_fname = "_".join( [ "L3_m", str(model), cname, str(cope), fsftemplate , contrast, roi] )
