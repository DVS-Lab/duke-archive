...
ISPARAMETRIC=SUB_ISPARAMETRIC_SUB


MAINDIR=${EXPERIMENT}/Analysis/Framing/FSL

if [ $ISPARAMETRIC -eq 1 ]; then
	preOUTPUT=${MAINDIR}/Level3_n50_DMN/${TEMPLATE}/Model${MODEL}_par
else
	preOUTPUT=${MAINDIR}/Level3_n50_DMN/${TEMPLATE}/Model${MODEL}
fi
mkdir -p $preOUTPUT
...
#removed subjects without resting data

	let N=$N+1
	if [ $ISPARAMETRIC -eq 1 ]; then
		FILENAME=${MAINDIR}/${SUBJ}/NoLapses_par/model${MODEL}.gfeat/cope${COPE}.feat/stats/cope1.nii.gz
	else
...
NEWFILE=${preOUTPUT}/L3_n50_${CNAME} #appending .fsf lower in the script
cd ${TEMPLATEDIR}
