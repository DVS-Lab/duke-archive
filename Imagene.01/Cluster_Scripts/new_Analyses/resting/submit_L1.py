...
template_f = file("L1_resting.sh")  #job template location (on head node)
experiment = "Imagene.01"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 150         #leave one in q to keep them moving through
min_jobs = 8                 #minimum number of jobs to keep in q even when crowded
...
goes = ["1", "2"]
whichstandards = [ "default", "new" ]
ROIs = ["PCC1", "PCC2", "PCC3"]

#PCC 1: Fox et al 2005 PNAS -- -2 -36 37 (tal) (intrinsically defined)
#-2.0202  -38.9734   38.2114

#PCC 2: Fox et al 2005 PNAS -- -5 -49 40 (tal?) meta-analysis
#-5.0505  -52.5133   40.7620

#PCC 3: Andrews-Hanna et al Neuron -- -1 -50 26 (tal)
#-1.0101  -52.8216   25.5093
...
#for each subject
for go in goes:
	for whichstandard in whichstandards:
		for ROI in ROIs:
			for subnum in subnums:
				...
				tmp_job_file = tmp_job_file.replace( "SUB_STANDARD_SUB", str(whichstandard) )
		
				#make fname and write job file to cwd
				tmp_job_fname = "_".join( ["L1_resting", ROI, subnum] )
