...
for PCC in 1 2 3; do
	...
	for MPFC in 1 2 3 4; do	
		MAINDIR=${EXPERIMENT}/Analysis/Resting_Default-Network
		MASKDIR=${MAINDIR}/ROIs
		
		COPE=${MAINDIR}/${SUBJ}/${SUBJ}_PCC${PCC}.feat
	
		MASK=${MAINDIR}/ROIs/MPFC${MPFC}.nii.gz
		MASKNAME=MPFC${MPFC}
		...
		${FSLDIR}/bin/featquery 1 ${COPE} 2 stats/cope3 stats/zstat3 ${MASKNAME}_FQ -s -b ${MASK}
