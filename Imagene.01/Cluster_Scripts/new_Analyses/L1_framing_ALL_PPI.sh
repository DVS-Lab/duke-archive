...
#FSLDATADIR2=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}/NoLapses/model${MODEL}

ANAT=$EXPERIMENT/Analysis/TaskData/${SUBJ}/${SUBJ}_anat_brain.nii.gz
DATA=${FSLDATADIR}/run${run}.feat/filtered_func_data.nii.gz
MAINOUTPUT=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}/corrected_NoLapses_PPI/model${MODEL}/${ROI}/${CON}
OUTPUT=${MAINOUTPUT}/run${run}
mkdir -p ${MAINOUTPUT}

OUTDIR=$EXPERIMENT/Analysis/Framing/Logs/DVS_October2011_ppi/L1_m${MODEL}_NEW_go_${GO}/$SUBJ
mkdir -p $OUTDIR
...
FILE_CHECK1=${OUTPUT}.feat/cluster_mask_zstat1.nii.gz
FILE_CHECK2=${OUTPUT}.feat/reg/example_func2standard.mat
if [ -e $FILE_CHECK1 -a -e $FILE_CHECK2 ]; then
...
LOCAL=${FSLDATADIR}/run${run}.feat/${ROI}_local_filtered.txt
rm -rf $LOCAL

REF=${FSLDATADIR}/run${run}.feat/reg/example_func.nii.gz
MATRIX=${FSLDATADIR}/run${run}.feat/reg/standard2example_func.mat
FLIRTOUTPUT=${FSLDATADIR}/run${run}.feat/${ROI}_native
FLIRTINPUT=$EXPERIMENT/Analysis/Framing/FSL/ROIs/${ROI}.nii.gz
WB=${FSLDATADIR}/run${run}.feat/mask.nii.gz
...
#47725_char-self_constant_run1.txt
...
TEMPLATE=$EXPERIMENT/Analysis/Framing/Templates/L1_template_05EVs_PPI_new.fsf
