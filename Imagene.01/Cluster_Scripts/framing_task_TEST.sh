...
OUTDIR=$EXPERIMENT/Analysis/Framing/Logs/DVS/$SUBJ
mkdir -p $OUTDIR
...
OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model1d_run${run}
...
TEMPLATE=$EXPERIMENT/Analysis/Framing/Templates/template_model1_TEST.fsf
sed -e 's@DATA@'$DATA'@g' \
-e 's@ANAT@'$ANAT'@g' \
-e 's@OUTPUT@'$OUTPUT'@g' \
-e 's@EV1@'$EV1'@g' \
...
-e 's@EV6@'$EV6'@g' \
<$TEMPLATE> ${MAINOUTPUT}/run${run}/${SUBJ}_model1d_run${run}.fsf
...
if [ -d $OUTPUT.feat ]; then
	...
	$FSLDIR/bin/feat ${MAINOUTPUT}/run${run}/${SUBJ}_model1d_run${run}.fsf
fi
