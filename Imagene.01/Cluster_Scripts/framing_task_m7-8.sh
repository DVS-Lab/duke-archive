...
FSLEVDIR=$EXPERIMENT/Analysis/Framing/EVfiles2/model${MODEL}/${SUBJ}
ANAT=$EXPERIMENT/Analysis/TaskData/${SUBJ}/${SUBJ}_anat_brain.nii.gz
MAINOUTPUT=$EXPERIMENT/Analysis/Framing/FSL/${SUBJ}
OUTDIR=$EXPERIMENT/Analysis/Framing/Logs/DVS/L1_m${MODEL}_NEW/$SUBJ
mkdir -p $OUTDIR
...
EV6=${FSLEVDIR}/run${run}/${SUBJ}_cue_run${run}.txt

OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model${MODEL}_run${run}
#OUTPUT=${MAINOUTPUT}/run${run}/${SUBJ}_model${MODEL}_badTRs_run${run}
...
CONFOUNDEVS=${FSLDATADIR}/run${run}.feat/bad_timepoints_FAKE.txt
if [ -e $CONFOUNDEVS ]; then
	TEMPLATE=$EXPERIMENT/Analysis/Framing/Templates/L1_m7-8_template_badTRs.fsf
	sed -e 's@DATA@'$DATA'@g' \
	-e 's@ANAT@'$ANAT'@g' \
	-e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@EV1@'$EV1'@g' \
	...
	<$TEMPLATE> ${MAINOUTPUT}/run${run}/${SUBJ}_model${MODEL}_run${run}.fsf
else
	TEMPLATE=$EXPERIMENT/Analysis/Framing/Templates/L1_m7-8_template.fsf
	sed -e 's@DATA@'$DATA'@g' \
	-e 's@ANAT@'$ANAT'@g' \
	-e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@EV1@'$EV1'@g' \
...
if [ -d $OUTPUT.feat ]; then
	...
	$FSLDIR/bin/feat ${MAINOUTPUT}/run${run}/${SUBJ}_model${MODEL}_run${run}.fsf
fi
