...
if [ $JUST_ANAT -eq 1 ]; then
	...
	for RUNS in `seq $RUNS`; do
		...
		if [ $RUNS -ge $GAMBLING ] && [ $RUNS -le $GAMBLING_STOP ];then
			...
			extractsliceorder --fsl --overwrite reoriented_run${RUNS}.bxh ${GOUTPUTDIR}/so_run${R3}.txt
			rm -rf ${GOUTPUTDIR}/temp_file.txt
		...
		if [ $RUNS -eq $REST ];then
			...
			extractsliceorder --fsl --overwrite reoriented_run${RUNS}.bxh ${ROUTDIR}/so_run${R4}.txt
			rm -rf ${ROUTDIR}/temp_file.txt
...
OUTDIR=${EXPERIMENT}/Analysis/TaskData/Job_logs_NEW_stc_fix
mkdir -p $OUTDIR
