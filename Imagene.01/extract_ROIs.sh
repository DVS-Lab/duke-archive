...
MAINDIR=${EXPERIMENT}/Analysis/TaskData/${SUBJ}/Resting
SUBJDIR=${MAINDIR}
MAINOUTPUT=${SUBJDIR}/PreStatsOnly/Smooth_${SMOOTH}mm
OUTPUTREAL=${MAINOUTPUT}/run${RUN}.feat
ROIDIR=${EXPERIMENT}/Analysis/TaskData/ROIs
...
cd $OUTPUTREAL
rm new_mask.nii
DATA=${MAINOUTPUT}/run${RUN}.feat/new_filtered_func_data.nii.gz
