...
FSLDATADIR=$EXPERIMENT/Analysis/TaskData/${SUBJ}/Resting/PreStatsOnly/Smooth_6mm
...
mkdir -p $MAINOUTPUT
OUTPUT=${MAINOUTPUT}/${SUBJ}_VMPFC

OUTDIR=$EXPERIMENT/Analysis/Resting_VMPFC/Logs/${SUBJ}
mkdir -p $OUTDIR
...
#make pVMPFC
PVMPFC=${FSLDATADIR}/run1.feat/pVMPFC_filtered.txt
REF=${FSLDATADIR}/run1.feat/reg/example_func.nii.gz
MATRIX=${FSLDATADIR}/run1.feat/reg/standard2example_func.mat
FLIRTOUTPUT=${FSLDATADIR}/run1.feat/pVMPFC_native
...
if [ -e $CONFOUNDEVS ]; then
	TEMPLATE=$EXPERIMENT/Analysis/Resting_VMPFC/Templates/resting_VMPFC_badTRs.fsf
	sed -e 's@DATA@'$DATA'@g' \
	-e 's@ANAT@'$ANAT'@g' \
	-e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@GLOBAL@'$GLOBAL'@g' \
	...
	TEMPLATE=$EXPERIMENT/Analysis/Resting_VMPFC/Templates/resting_VMPFC.fsf
	sed -e 's@DATA@'$DATA'@g' \
	-e 's@ANAT@'$ANAT'@g' \
	-e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@GLOBAL@'$GLOBAL'@g' \
