...
template_f = file("L1_resting_NL.sh")  #job template location (on head node)
experiment = "Imagene.01"    #experiment name for qsub
nodes = 20                   #number of nodes on cluster
maintain_n_jobs = 21         #leave one in q to keep them moving through
min_jobs = 6                 #minimum number of jobs to keep in q even when crowded
...
for go in goes:
	for subnum in subnums:
		...
		tmp_job_fname = "_".join( ["L1_resting_VMPFC", subnum, go] )
