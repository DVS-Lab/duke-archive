...
if [ $FNIRT -eq 1 ]; then
	MAINOUTPUT=${MAINDIR}/FSL/${SUBJ}/PreStats_FNIRT_spline/Smooth_${SMOOTH}mm
	mkdir -p $MAINOUTPUT
...
if [ -e $CHECKREG ]; then
	...
	for i in `seq 0 $NVOLS`; do
		...
		$FSLDIR/bin/applywarp --ref=standard --in=${MCFDATA}/tmp_data${I} --out=${MCFDATA}/data2standard${I} --warp=${MCFDATA}/all_warps${I} --interp=spline
		
	done
	
	fslmerge -tr ${MCFDATA}/normed_data ${MCFDATA}/data2standard* 1.96
	#then run the rest of preprocessing through the FEAT GUI: stc, BET, temporal filtering, and grand mean scaling
	
fi

NORMEDDATA=${MCFDATA}/data2standard.nii.gz

#finishing prestats: bet, filtering, grand mean intensity normalization, and melodic (just for kicks)
OUTPUT=${OUTPUT}.feat/prestats_phase2
NVOLUMES=`fslnvols ${CORRECTEDDATA}`
...
if [ -d ${OUTPUT}.ica/filtered_func_data.nii.gz ]; then
