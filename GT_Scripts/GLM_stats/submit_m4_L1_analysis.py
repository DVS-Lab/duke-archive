...
template_f = file("m4_L1_analysis.sh")  #job template location (on head node)
			...
			for smooth in smooths:
				for subnum in subnums:
					for run in runs:
						...
						tmp_job_fname = "_".join( ["HiResL1", subnum, runstr, fnirt, go] )
						...
						while n_jobs >= maintain_n_jobs: 
							...
							if n_jobs >= maintain_n_jobs: 
								...
								cmd = "qsub -l h_vmem=8G -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
