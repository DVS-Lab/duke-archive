...
if [ ! -e $FILE_TO_CHECK ]; then
	#rm -rf ${OUTPUTREAL}
	mv ${OUTPUTREAL} ${MAINOUTPUT}/FAILED_run${RUN}.ica
fi
if [ -d ${MAINOUTPUT}/FAILED_run${RUN}.ica ]; then
	rm -rf ${MAINOUTPUT}/FAILED_run${RUN}.ica
fi
...
DATA=${SUBJDIR}/${TASK}/trunc_run${RUN}.nii.gz
fslroi ${SUBJDIR}/${TASK}/run${RUN} ${SUBJDIR}/${TASK}/trunc_run${RUN}.nii.gz 0 200
...
if [ $SKIP -eq 1 ]; then
	...
	OUTDIR=${MAINDIR}/Logs/ica_new5/fsl_motion_outlier_skips/GO_${GO}
else
	...
	<melodic_IG2_150.fsf> ${MAINOUTPUT}/FEAT_0${RUN}.fsf
...
#smb://munin.biac.duke.edu/Huettel/Imagene.02/Analysis/TaskData/10168/Resting/MELODIC_FLIRT/Smooth_6mm/run1.ica/reg
		if [ -d ${OUTPUTREAL}/reg ]; then
			mv ${OUTPUTREAL}/reg ${OUTPUTREAL}/old_reg
			cp -r ${MAINDIR}/${SUBJ}/${TASK}/MELODIC_FLIRT/Smooth_${SMOOTH}mm/run${RUN}.ica/reg ${OUTPUTREAL}/reg
		fi
		
	
		# letting FSL do everything. use defaults for better/bigger mask (DVS: 5/15/10)
...
# 		$FSLDIR/bin/fslmaths filtered_func_data.nii.gz -mas new_mask.nii.gz new_filtered_func_data
	...
	if [ -e ${OUTPUTREAL}/bad_timepoints.txt ]; then
		...
		OUTDIR=${MAINDIR}/Logs/ica_new5/fsl_motion_outlier_SUCCESS/GO_${GO}
	else
		fsl_motion_outliers ${DATA} ${NDISDAQS} ${OUTPUTREAL}/bad_timepoints.txt
		if [ -e ${OUTPUTREAL}/bad_timepoints.txt ]; then
			OUTDIR=${MAINDIR}/Logs/ica_new5/fsl_motion_outlier_SUCCESS/GO_${GO}
		else
			OUTDIR=${MAINDIR}/Logs/ica_new5/fsl_motion_outlier_FAIL/GO_${GO}
		fi
	fi
fi

rm -rf ${SUBJDIR}/${TASK}/trunc_run${RUN}.nii.gz
