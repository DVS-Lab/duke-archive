...
ICNAM=$3 #SN_minus_VTA or VTA_minus_SN
NCONTRASTS=1 #how many contrasts are in your design.con file?
...
MAINDIR=/mnt/BIAC/munin.dhe.duke.edu/Huettel/Imagene.02/Analysis/TaskData/groupICA_AU/DVS/for_Maheen
OUTPUT=$MAINDIR/${ICAOUTPUT}.ica/${DROUTPUT}

#IC=`$FSLDIR/bin/zeropad $ICNUM 4`

#randomise command
$FSLDIR/bin/randomise -i $OUTPUT/$ICNAM -o $OUTPUT/dr_stage3b_$ICNAM -m $OUTPUT/mask -d $OUTPUT/design.mat -t $OUTPUT/design.con -n 10000 -T -V
##$FSLDIR/bin/randomise -i $OUTPUT/$ICNAM -o $OUTPUT/dr_stage3b_$ICNAM -m $OUTPUT/mask -d $OUTPUT/design.mat -t $OUTPUT/design.con -n 10000 -x -V

#threshold output
for C in `seq $NCONTRASTS`; do
	OUTIMAGE=dr_stage3b_${ICNAM}_tfce_corrp_tstat${C}
	...
	#OUTIMAGE=dr_stage3b_${ICNAM}_vox_corrp_tstat${C}
	##$FSLDIR/bin/fslmaths $OUTPUT/$OUTIMAGE -thr 0.95 $OUTPUT/thresh_${OUTIMAGE}
	#overlay 1 0 ${FSLDIR}/data/standard/MNI152_T1_2mm_brain -a $OUTPUT/${OUTIMAGE} .95 1 $OUTPUT/renderedthresh_${OUTIMAGE}
	#slicer $OUTPUT/renderedthresh_${OUTIMAGE} -S 2 750 $OUTPUT/renderedthresh_${OUTIMAGE}.png
