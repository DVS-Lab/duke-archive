...
template_f = file("MVPA_redos.sh") 	#job template location (on head node)
...
classifiers = ['SMLR3', 'LinearNuSVMC', 'RbfNuSVMC' ]
masks = [ "old", "new" ]
combos = [ "3", "4" ]
ID = "0"
			...
			for mask in masks:
				for testname in testnames:
					...
					for i in range(1,N_vars+1):
						...
						combo_c = ''.join( [ "redo_c", combo ] )
						tmp_job_fname = "_".join( [combo_c, "MVPA", i_str, mask, testname, datatype, classifier] )
