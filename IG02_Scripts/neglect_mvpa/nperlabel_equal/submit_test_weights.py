...
template_f = file("test_weights.sh") 	#job template location (on head node)
experiment = "Imagene.02"   		#experiment name for qsub
nodes = 400                  		#number of nodes on cluster
maintain_n_jobs = 5        		#leave one in q to keep them moving through
...
for ROI in ROIs:
	for datatype in datatypes:
		...
		tmp_job_fname = "_".join( ["test", ROI, datatype] )
		...
		while n_jobs >= maintain_n_jobs: 
			...
			if n_jobs >= maintain_n_jobs: 
				...
				cmd = "qsub -l h_vmem=22G,h_rt=01:30:00 -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
