...
template_f = file("fix_fail.sh") 	#job template location (on head node)
...
#ROIs = [ '012', '014', '016', '018', '030', '062', '064', '066', '082', '084', '086', '088' ]
ROIs = [ '016', '018' ]
	...
	for ls_type in ls_types:
		for ROI in ROIs:
			for datatype in datatypes:
				...
				tmp_job_file = tmp_job_file.replace("SUB_LSTYPE_SUB", ls_type )
				
				#make fname and write job file to cwd
				tmp_job_fname = "_".join( ["fix_fail", ROI, datatype, ls_type, str(loop) ] )
