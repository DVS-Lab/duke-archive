...
MAINOUTPUT=${MAINDIR}/PythonFiles/$NCOMBO
mkdir -p $MAINOUTPUT

sed -e 's@MAINDIR@'$MAINDIR'@g' \
-e 's@SED_ROILIST_SED@'"$ROI_LIST"'@g' \
-e 's@SED_NCOMBO_SED@'$NCOMBO'@g' \
<template_PYMVPA_ROI_equal_c4_197.py> ${MAINOUTPUT}/mvpa_combo${NCOMBO}_${ID}_equal_${PERM}.py

python ${MAINOUTPUT}/mvpa_combo${NCOMBO}_${ID}_equal_${PERM}.py
rm ${MAINOUTPUT}/mvpa_combo${NCOMBO}_${ID}_equal_${PERM}.py
...
OUTDIR=$MAINDIR/c4Logs/equal/$NCOMBO
mkdir -p $OUTDIR
