...
template_f = file("MVPA_equal_voxel_repeats.sh") 	#job template location (on head node)
	...
	for combo in combos:
		for classifier in classifiers:
			...
			for i in range(1,N_vars+1):
				...
				tmp_job_file = tmp_job_file.replace("SUB_ID_SUB",str(i_str) )
				...
				tmp_job_fname = "_".join( [ "vox_MVPA", i_str, classifier, repeat] )
