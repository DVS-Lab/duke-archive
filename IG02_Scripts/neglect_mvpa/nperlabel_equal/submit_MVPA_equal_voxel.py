...
template_f = file("MVPA_equal_voxel.sh") 	#job template location (on head node)
...
for combo in combos:
	for classifier in classifiers:
		...
		if comboN > 1:
			...
			fname = ("/home/%s/Imagene.02/neglect_mvpa/nperlabel_equal/voxel_list.txt") % (username)
		...
		for i in range(1,N_vars+1):
			...
			tmp_job_fname = "_".join( [ "vox_MVPA", i_str, classifier] )
