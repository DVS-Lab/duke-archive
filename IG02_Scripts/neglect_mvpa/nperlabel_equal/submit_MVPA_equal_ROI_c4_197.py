...
template_f = file("MVPA_equal_ROI_c4_197.sh") 	#job template location (on head node)
...
max_run_time = 99999          		#maximum time any job is allowed to run in minutes
...
for combo in combos:
	...
	if comboN > 1:
		...
		fname = ("/home/%s/Imagene.02/neglect_mvpa/nperlabel_equal/selected197_ROIs_c%s_wJuelich_%smask.txt") % (username,combo,mask)
	...
	for i in range(1,N_vars+1):
		...
		tmp_job_fname = "_".join( [combo_c, "MVPA", i_str] )
		...
		while n_jobs >= maintain_n_jobs: 
			...
			if n_jobs >= maintain_n_jobs: 
				...
				cmd = "qsub -l h_vmem=5G,h_rt=00:60:00 -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
