...
for ROI in ROIs:
	for perm in perms:
		for datatype in datatypes:
			...
			tmp_job_file = tmp_job_file.replace("SUB_PERM_SUB", perm_str )
			
			#make fname and write job file to cwd
			tmp_job_fname = "_".join( ["sel_c3", "MVPA", ROI, datatype, perm_str] )
			...
			while n_jobs >= maintain_n_jobs: 
				...
				if n_jobs >= maintain_n_jobs: 
					...
					cmd = "qsub -l h_vmem=3G,h_rt=01:30:00 -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
