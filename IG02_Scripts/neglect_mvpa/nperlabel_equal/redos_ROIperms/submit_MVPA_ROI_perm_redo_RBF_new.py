...
template_f = file("MVPA_ROI_perm_redo_RBF_new.sh") 	#job template location (on head node)
experiment = "Imagene.02"   		#experiment name for qsub
nodes = 400                  		#number of nodes on cluster
maintain_n_jobs = 350        		#leave one in q to keep them moving through
		...
		for rep in reps:
			for perm in perms:
				for classifier in classifiers:
					...
					tmp_job_file = tmp_job_file.replace("SUB_REP_SUB", str(rep) )
					tmp_job_fname = "_".join( [ "redo", combo, classifier, datatype, str(perm), str(rep)] )
					...
					while n_jobs >= maintain_n_jobs: 
						...
						if n_jobs >= maintain_n_jobs: 
							...
							cmd = "qsub -l h_vmem=2G -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
