	...
	for perm in perms:
		...
		for classifier in classifiers:
			for datatype in datatypes:
				...
				fname = ("/home/%s/Imagene.02/neglect_mvpa/nperlabel_equal/redos_ROIperms/anatROIs_c%s_wJuelich_newmask2.txt") % (username,combo)
				if os.path.isfile(fname):
					ROI_f = open(fname,"r")
					ROI_list = ROI_f.readlines()
					ROI_f.close()
					...
					for line in ROI_list:
						...
						for ROI in ROIs:
							ROI_str = str(ROI)
							ROI_str = ROI_str.zfill(3)
					
								
							fname = ("/home/%s/Imagene.02/neglect_mvpa/nperlabel_equal/redos_ROIperms/missing_lists/combo%s/perm%s_roi_%s_combo%s_%s_CV_%s_missing.txt") % (username,combo,perm_str,ROI_str,combo,classifier,datatype)
							...
							for line in combo_list:
								...
								tmp_job_file = tmp_job_file.replace("SUB_REPEAT_SUB", repeat )
								tmp_job_fname = "_".join( [ "redo", combo, classifier, datatype, ROI_str, repeat, perm] )
