...
template_f = file("get_missing_rois_c3.sh") 	#job template location (on head node)
				...
				if os.path.isfile(fname):
					...
					for line in ROI_list:
						...
						if int(combo[0]) > 1:
							...
							tmp_job_fname = "_".join( [ "missing", combo, classifier, datatype, ROI_str, str(perm)] )
							...
							for ROI in ROIs:
								...
								tmp_job_fname = "_".join( [ "missing", combo, classifier, datatype, ROI_str, repeat] )
								tmp_job_f = file( tmp_job_fname, "w" )
								tmp_job_f.write(tmp_job_file)
								tmp_job_f.close()
