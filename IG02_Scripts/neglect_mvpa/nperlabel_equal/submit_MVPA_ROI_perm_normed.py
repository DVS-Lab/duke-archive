...
template_f = file("MVPA_ROI_perm.sh") 	#job template location (on head node)
...
datatypes = [ "normed" ] #missing raw from 821 to 1000
#classifiers = [ 'LinearNuSVMC', 'RbfNuSVMC' ]
classifiers = [ 'RbfNuSVMC' ]
perms = range(913,1001)
combos = [ "3" ]
ID = "0"
mask = "new"
		...
		for classifier in classifiers:
			for datatype in datatypes:
				...
				if comboN > 1:
					...
					fname = ("/home/%s/Imagene.02/neglect_mvpa/nperlabel_equal/anatROIs_c%s_wJuelich_%smask2.txt") % (username,combo,mask)
				...
				for i in range(1,N_vars+1):
					...
					tmp_job_file = tmp_job_file.replace("SUB_PERM_SUB", permstr )
					...
					combo_c = ''.join( [ "perm_c", combo ] )
					tmp_job_fname = "_".join( [combo_c, "MVPA", i_str, datatype, classifier, permstr] )
