...
template_f = file("check_weights.sh") 	#job template location (on head node)
...
for ROI in ROIs:
	for datatype in datatypes:
		...
		tmp_job_fname = "_".join( ["missing", ROI, datatype] )
		...
		while n_jobs >= maintain_n_jobs: 
			...
			if n_jobs >= maintain_n_jobs: 
				...
				cmd = "qsub -l h_vmem=5G,h_rt=01:30:00 -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
