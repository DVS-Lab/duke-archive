...
template_f = file("MVPA_equal_weights_all.sh") 	#job template location (on head node)
...
max_run_time = 75          		#maximum time any job is allowed to run in minutes
		...
		for datatype in datatypes:
			for testname in testnames:
				...
				for i in range(1,N_vars+1):
					...
					tmp_job_fname = "_".join( [combo_c, "MVPA", i_str, testname, datatype, perm_str] )
					...
					while n_jobs >= maintain_n_jobs: 
						...
						if n_jobs >= maintain_n_jobs: 
							...
							cmd = "qsub -l h_vmem=3G,h_rt=00:30:00 -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
