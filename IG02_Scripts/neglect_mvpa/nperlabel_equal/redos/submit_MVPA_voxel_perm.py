...
template_f = file("MVPA_voxel_perm.sh") 	#job template location (on head node)
...
voxels = [ "074_047_044", "081_056_040" ]
...
for voxel in voxels:
	...
	fname = ("/home/%s/Imagene.02/neglect_mvpa/nperlabel_equal/redos/perm_vox_%s_CVs_LinearNuSVMC_normed_2D_missing.txt") % (username,voxel)
	...
	for line in combo_list:
		...
		tmp_job_file = tmp_job_file.replace("SUB_VOXEL_SUB", voxel )

		#make fname and write job file to cwd
		perm_str = str(perm)
		perm_str = perm_str.zfill(4)
		
		tmp_job_fname = "_".join( [ "vox_MVPA", repeat, perm_str] )
