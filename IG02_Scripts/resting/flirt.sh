...
for TYPE in 1 2 3 4; do
	...
	if [ $SKIP -eq 1 ]; then
		...
		OUTDIR=$EXPERIMENT/Analysis/Resting_Default-Network/Logs/skipped_flirt
	else
	
		#make standardized z-maps
		REF=${OUTPUT}.feat/reg/standard
		MATRIX=${OUTPUT}.feat/reg/example_func2standard.mat
		FLIRTOUTPUT=${OUTPUT}.feat/reg_standard/stats/zstat3
		FLIRTINPUT=${OUTPUT}.feat/stats/zstat3
		
		if [ ! -e $FLIRTOUTPUT ]; then
			flirt -in ${FLIRTINPUT} -ref ${REF} -applyxfm -init ${MATRIX} -out ${FLIRTOUTPUT}
		fi
	
		OUTDIR=$EXPERIMENT/Analysis/Resting_Default-Network/Logs/did_flirt
