...
if [ $SKIP -eq 1 ]; then
	...
	OUTDIR=${MAINDIR}/Logs/reg/GO_${GO}_reg_skipped
else
	if [ $FNIRT -eq 1 ]; then
		CHECKREG=${OUTPUT}.ica/reg/example_func2standard.png
		if [ -e $CHECKREG ]; then
			echo "registration is complete..."
			OUTDIR=${MAINDIR}/Logs/reg/GO_${GO}_reg_success
		else
			OUTDIR=${MAINDIR}/Logs/reg/GO_${GO}_reg_redo
			if [ $GO -gt 1 ]; then
				echo "registration fail... trying again..."
			fi
			REGDIR=${OUTPUT}.ica/reg
			if [ -d ${OUTPUT}.ica/reg ]; then
