...
TASKDIR=${MAINDIR}/${SUBJ}

QA_OUTPUT=${TASKDIR}/QA
if [ $GO -eq 1 ]; then
	rm -rf ${QA_OUTPUT}
fi
...
if [ $DO_QA -eq 1 ]; then
	...
	if [ -d $QA_OUTPUT ]; then
		cd ${QA_OUTPUT}
		OUTDIR=${EXPERIMENT}/Analysis/FSL/Logs/QA/success
	else
		OUTDIR=${EXPERIMENT}/Analysis/FSL/Logs/QA/fail
		echo "FAIL: $QA_OUTPUT"
	fi
	...
	OUTDIR=${EXPERIMENT}/Analysis/FSL/Logs/QA/skipped
fi
