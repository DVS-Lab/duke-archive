function compare_networks_ECN_run5_jake_dvs(~)
    maindir = pwd;
    my_Nics = 25;
    ...
    mask = fullfile(maindir,'run5_FLIRT','gica_unconfounded_tensor_DR_25dim.ica','mask.nii.gz'); 
        for i1 = 1:my_Nics %rows
            ...
            i1_str = sprintf('%04d',ic1); 
                ...
                split_melodic_IC_file2 = fullfile(maindir,'PNAS_3mm_components','IC_0007.nii.gz'); %This is my ECN template
                %keyboard
        ...
        title('DR')
        dlmwrite('ECN_corr_unconfounded_gica_tensor_DR_JY.csv',corr_mat,'delimiter',',','precision','%.6f')
end
