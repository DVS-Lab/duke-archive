    ...
    mask = fullfile(maindir,'run5_FLIRT','gica_DR_25dim.ica','mask.nii.gz'); 
        for i1 = 1:my_Nics %rows
            ...
            i1_str = sprintf('%04d',ic1); 
                split_melodic_IC_file1 = fullfile(maindir,'run5_FLIRT','gica_DR_25dim.ica',['IC_' i1_str '.nii.gz']);
        ...
        title('DR')
        dlmwrite('face-land_corr_run5_DR_JY.csv',corr_mat,'delimiter',',','precision','%.6f')
end
