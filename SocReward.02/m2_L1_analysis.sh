...
ANAT=${MAINDIR}/FSL/$SUBJ/${SUBJ}_anat_brain
if [ $FNIRT -eq 1 ]; then
	PRESTATSREGDIR=${MAINDIR}/FSL/${SUBJ}/MELODIC_FNIRT/Smooth_${SMOOTH}mm/run${RUN}.ica/reg
	MAINOUTPUT=${MAINDIR}/FSL/${SUBJ}/Model02_FNIRT/Smooth_${SMOOTH}mm
	mkdir -p ${MAINOUTPUT}
else
	PRESTATSREGDIR=${MAINDIR}/FSL/${SUBJ}/MELODIC_FLIRT/Smooth_${SMOOTH}mm/run${RUN}.ica/reg
	MAINOUTPUT=${MAINDIR}/FSL/${SUBJ}/Model02_FLIRT/Smooth_${SMOOTH}mm
	mkdir -p ${MAINOUTPUT}
fi


PRESTATSDATA=${MAINDIR}/FSL/${SUBJ}/MELODIC_FLIRT/Smooth_${SMOOTH}mm/run${RUN}.ica
DATA=${PRESTATSDATA}/filtered_func_data.nii.gz

OUTPUT=${MAINOUTPUT}/run${RUN}
CONFOUNDEVSFILE=${PRESTATSDATA}/bad_timepoints.txt
...
EVFILES=${MAINDIR}/FSL/EV_files/Model02/${SUBJ}/run${RUN}
...
FDIMAGE=${EVFILES}/face_delay_image.txt
FVIMAGE=${EVFILES}/face_value_image.txt
LDIMAGE=${EVFILES}/land_delay_image.txt
LVIMAGE=${EVFILES}/land_value_image.txt



TEMPLATE=${MAINDIR}/FSL/Templates/L1_model02.fsf
...
-e 's@LANDC@'$LANDC'@g' \
-e 's@FACEC@'$FACEC'@g' \
-e 's@FDIMAGE@'$FDIMAGE'@g' \
-e 's@FVIMAGE@'$FVIMAGE'@g' \
-e 's@LDIMAGE@'$LDIMAGE'@g' \
-e 's@LVIMAGE@'$LVIMAGE'@g' \
