...
template_f = file("level3.sh")  #job template location (on head node)
...
min_jobs = 2                 #minimum number of jobs to keep in q even when crowded
n_fake_jobs = 4               #during business hours, pretend there are extra jobs to try and leave a few spots open
sleep_time = 45              #pause time (sec) between job count checks
max_run_time = 720           #maximum time any job is allowed to run in minutes
max_run_hours = 96	#maximum number of hours submission script can run
...
#"L3_cross_studyF", "L3_cross_study", 
#TEMPLATES = ["L3_BIS-Cognitive", "L3_BIS-Motor", "L3_BIS-Nonplanning", "L3_DSRA-Financial", "L3_DSRA-Ethical", "L3_DSRA-HS", "L3_DSRA-Rec", "L3_DSRA-Soc", "L3_Exchanges", "L3_Cog-Int_FI", "L3_Cog-Int_NFC", "L3_normal", "L3_paired_T-test", "L3_PAL_Total", "L3_PAL_Strangers", "L3_PAL_Friends"]
TEMPLATES = ["L3_paired_T-test"]
MODELS = ["Model_1"]
TASKS = ["Passive"]
CORRECTIONS = ["0"]
GOES = ["1"]
...
daemonize('/dev/null',os.path.join(start_dir,'daemon2.log'),os.path.join(start_dir,'daemon2.log'))
sys.stdout.close()
os.chdir(start_dir)
temp=time.localtime()
...
log_name=os.path.join(start_dir,'daemon2.log')
log=file(log_name,'w')
					...
					for TEMPLATE in TEMPLATES:
						for MODEL in MODELS:
							for TASK in TASKS:
								...
								tmp_job_file = tmp_job_file.replace( "SUB_TEMPLATE", str(TEMPLATE) )
								tmp_job_file = tmp_job_file.replace( "SUB_CORRECTION", str(CORRECTION) )
								tmp_job_file = tmp_job_file.replace( "SUB_FIXED", str(FIXED) )
								...
								tmp_job_fname = "_".join( [MODEL, TASK, ISO, DENOISED, TEMPLATE] )
