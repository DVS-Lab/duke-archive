...
if [ "$TEMPLATE" == "L3_paired_T-test" ] && [ ! "$TASK" == "Passive" ]; then
	...
	if [ $FIXED -eq 1 ]; then
		...
		if [ $CORRECTION -eq 1 ]; then
			MAINOUTPUTPRE=${MAINDIR}/ThirdLevel6_${ICA}/FEcorrected
			PTHRESH=0.05
			THRESHTYPE=3
		else
			MAINOUTPUTPRE=${MAINDIR}/ThirdLevel6_${ICA}/FEuncorrected
			PTHRESH=0.005
			THRESHTYPE=1
		fi
	else
		L3_TEMP=level3
		if [ $CORRECTION -eq 1 ]; then
			MAINOUTPUTPRE=${MAINDIR}/ThirdLevel6_${ICA}/corrected
			PTHRESH=0.05
			THRESHTYPE=3
		else
			MAINOUTPUTPRE=${MAINDIR}/ThirdLevel6_${ICA}/uncorrected
			PTHRESH=0.005
			THRESHTYPE=1
		fi
	fi
