...
template_f = file("level3_fixed.sh")  #job template location (on head node)
experiment = "SocReward.03"    #experiment name for qsub
nodes = 20                   #number of nodes on cluster
...
TASKS = ["Combo_Level2", "PassiveCued", "Passive"]
CORRECTIONS = ["1", "0"]
GOES = ["1", "2"]
		...
		for TEMPLATE in TEMPLATES:
			for MODEL in MODELS:
				for TASK in TASKS:
					...
					tmp_job_fname = "_".join( [MODEL, TASK, GO] )
