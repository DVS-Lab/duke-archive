...
#for COPES in `seq $NCOPES`; do
	...
	#for LIST in "M8_aINS.hdr M8_aINS" "M8_caudate.hdr M8_caudate" "M8_dorsalStr.hdr M8_dorsal" "M8_vStr.hdr M8_vStr" "M8_vmPFC.hdr M8_vmPFC"; do
	#	set -- $LIST
	
	MASKDIR=${MAINDIR}/ROIs_anatomical
	cd $MASKDIR
	ls -1 *.nii.gz > roifile

		cat roifile|while read LINE; do
		
		MASKNAME=$LINE
		MASK=${MAINDIR}/ROIs_anatomical/${MASKNAME}
...
OUTDIR=$MAINDIR/Featquery_Logs/${MODEL}
mkdir -p $OUTDIR
