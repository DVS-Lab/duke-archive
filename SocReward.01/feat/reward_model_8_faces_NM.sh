...
MAINOUTPUT=${MAINDIR}/${SUBJ}_Model8_faces_NoMotor_${SMOOTH}mm_ST_TD
...
cd ${TEMPLATEDIR}
let "run2=${run}-1"
POSITIVE=${FSLEVDIR}/Run${run2}/Run${run2}_Hot_${SUBJ}.txt
NEGATIVE=${FSLEVDIR}/Run${run2}/Run${run2}_Not_${SUBJ}.txt
#POSITIVE=${FSLEVDIR}/Run${run2}/Run${run2}_Gain_${SUBJ}.txt
#NEGATIVE=${FSLEVDIR}/Run${run2}/Run${run2}_Loss_${SUBJ}.txt

NEUTRAL=${FSLEVDIR}/Run${run2}/Run${run2}_NeutralFace_${SUBJ}.txt
#NEUTRAL=${FSLEVDIR}/Run${run2}/Run${run2}_NeutralMoney_${SUBJ}.txt
...
for i in 'reward_model_8_template_NoMotor_TD.fsf'; do
	sed -e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@POSITIVE@'$POSITIVE'@g' \
	-e 's@NEGATIVE@'$NEGATIVE'@g' \
