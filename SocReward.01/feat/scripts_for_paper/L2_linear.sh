...
SUBJ=SUB_SUBNUM_VAR
OPTION=SUB_OPTION_VAR
GO=SUB_GO_VAR


MAINDIR=${EXPERIMENT}/Analysis/Cluster/PassiveTask
MAINDIR2=${MAINDIR}/${SUBJ}/${SUBJ}_Positive_Linear_all

OUTPUT=${MAINDIR2}/${SUBJ}_2ndlevel
...
if [ $SUBJ -eq 33732 ]; then
	...
	TEMPLATE=level2_template33732_Linearall_positive_NEW.fsf
	cd $TEMPLATEDIR
	...
	INPUT1=${MAINDIR2}/${SUBJ}_run4.feat
	INPUT2=${MAINDIR2}/${SUBJ}_run3.feat
	INPUT4=${MAINDIR2}/${SUBJ}_run5.feat
	INPUT5=${MAINDIR2}/${SUBJ}_run6.feat
	...
	TEMPLATE=level2_template_Linearall_positive_NEW.fsf
	cd $TEMPLATEDIR
	for i in $TEMPLATE; do
	sed -e 's@OUTPUT@'$OUTPUT'@g' \
	    -e 's@INPUT1@'$INPUT1'@g' \
...
OUTDIR=$MAINDIR/Dec09_L2_logs_v3
mkdir -p $OUTDIR
