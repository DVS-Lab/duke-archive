...
template_f = file("FQ_basic.sh")  #job template location (on head node)
...
for cope in copes:
	for go in goes:
		...
		for subnum in subnums:
			...
			tmp_job_file = tmp_job_file.replace( "SUB_COPENUM_VAR", str(cope) )
	
			#make fname and write job file to cwd
			tmp_job_fname = "_".join( ["FQ", subnum, str(cope), go] )
