...
MAINOUTPUT=${SUBJDIR}/PreStatsOnly/${SUBJ}_prestats_Smooth_${SMOOTH}mm
...
for F in *.nii.gz; do
	...
	fslmaths ${NEWDATA} -mas ${OUTPUT} ${OUTPUTDIR4}/DATA_func2std_${MASKNAME}
...
fslmeants -i ${OUTPUTDIR2}/DATA_std2mni_${MASKNAME} -o ${OUTPUTDIR5}/${SUBJ}_region117.txt -m ${MASK}
...
fslmeants -i ${OUTPUTDIR4}/DATA_func2std_${MASKNAME} -o ${OUTPUTDIR6}/${SUBJ}_region117.txt -m ${OUTPUT}

cd $OUTPUTREAL
rm -f filtered_func_data.nii.gz
rm -f ${NEWDATA2}.nii.gz

OUTDIR=${MAINDIR}/ExtractROIs_Logs2
mkdir -p $OUTDIR
