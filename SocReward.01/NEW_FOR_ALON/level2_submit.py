...
template_f = file("level2.sh")  #job template location (on head node)
experiment = "SocReward.01"    #experiment name for qsub
nodes = 24                   #number of nodes on cluster
maintain_n_jobs = 25         #leave one in q to keep them moving through
min_jobs = 55                 #minimum number of jobs to keep in q even when crowded
n_fake_jobs = 15               #during business hours, pretend there are extra jobs to try and leave a few spots open
sleep_time = 2              #pause time (sec) between job count checks
max_run_time = 60           #maximum time any job is allowed to run in minutes
...
#subnums = ["32953", "32958", "32976", "32984", "33035", "33045", "33064", "33082", "33288", "33302", "33135", "33402", "33456", "33642", "33732", "33746", "33754", "33757", "33771", "33784", "33467", "33744", "32918"]
subnums = ["32953"]

#OPTIONS = ["PERM1", "PERM2", "PERM3", "PERM4", "OPTION_0.5", "OPTION_1", "OPTION_0.25", "OPTION_1.5", "OPTION_2", "OPTION_20", "OPTION_10", "OPTION_5"]
#OPTIONS = ["PERM1", "PERM2", "PERM3", "OPTION_0.5", "OPTION_1", "OPTION_0.25", "OPTION_1.5", "OPTION_2", "PERM4", "OPTION_5", "OPTION_10", "OPTION_20"]
OPTIONS = ["4"]
AR1S = ["1"]
GOES = ["1"]
SMOOTHS = ["6"];
		...
		for GO in GOES:
			...
			for subnum in subnums:
				...
				for OPTION in OPTIONS:
					...
					tmp_job_file = tmp_job_file.replace( "SUB_GO_VAR", str(GO) )
					tmp_job_file = tmp_job_file.replace( "SUB_SUBNUM_VAR", str(subnum) )
					tmp_job_file = tmp_job_file.replace( "SUB_OPTION_VAR", str(OPTION) )
					tmp_job_file = tmp_job_file.replace( "SUB_AR1_VAR", str(AR1) )
					tmp_job_file = tmp_job_file.replace( "SUB_SMOOTH_VAR", str(SMOOTH) )

					#make fname and write job file to cwd
					tmp_job_fname = "_".join( ["L2", subnum, OPTION, GO, AR1, SMOOTH] )
