...
MAINOUTPUT=${SUBJDIR}/NEW_ANALYSES/${SUBJ}_FEAT_${SPACE}_${SMOOTH}mm_PERM${PERM}_noAR1_nofilt
mkdir -p ${MAINOUTPUT}
...
ANAT=${SUBJDIR}/anat01_brain.nii.gz
DATA=${MAINDIR}/NEWDATA/smooth${SMOOTH}mm/permutation${PERM}/${SPACE}/${SUBJ}/data_run${RUN}.nii.gz
OUTPUT=${MAINOUTPUT}/run${RUN}

TEMPLATEDIR=${MAINDIR}/Templates

cd ${TEMPLATEDIR}
if [ ${RUN} -eq 1 ] || [ ${RUN} -eq 3 ] || [ ${RUN} -eq 5 ]; then
	TEMPLATE=stats1_noAR1_nofilt.fsf
else
	TEMPLATE=stats2_noAR1_nofilt.fsf
fi

REGRESSOR=${MAINDIR}/EVfiles/$SUBJ/${SUBJ}_run${RUN}.txt
