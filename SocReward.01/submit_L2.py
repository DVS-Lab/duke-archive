...
template_f = file("L2_paired.sh")  #job template location (on head node)
experiment = "SocReward.01"    #experiment name for qsub
nodes = 15                   #number of nodes on cluster
maintain_n_jobs = 10         #leave one in q to keep them moving through
...
for go in goes:
	for con_name in con_names:
		for subnum in subnums:
			...
			tmp_job_file = tmp_job_file.replace( "SUB_NAME_VAR", str(con_name) )
	
	
			#make fname and write job file to cwd
			tmp_job_fname = "_".join( ["L2", "paired", subnum, con_name, go] )
