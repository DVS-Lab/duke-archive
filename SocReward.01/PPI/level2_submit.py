...
template_f = file("secondlvl_feat2.sh")  #job template location (on head node)
experiment = "SocReward.01"    #experiment name for qsub
nodes = 15                   #number of nodes on cluster
maintain_n_jobs = 15         #leave one in q to keep them moving through
min_jobs = 5                 #minimum number of jobs to keep in q even when crowded
n_fake_jobs = 18               #during business hours, pretend there are extra jobs to try and leave a few spots open
sleep_time = 10              #pause time (sec) between job count checks
max_run_time = 100           #maximum time any job is allowed to run in minutes
...
subnums = ["32918", "32953", "32958", "32976", "32984", "33035", "33045", "33064", "33082", "33288", "33302", "33402", "33456", "33642", "33732", "33746", "33754", "33757", "33771", "33784", "33744", "33467", "33135"]
#subnums = ["33732"]
models = ["Linear_Increase", "Linear_Decrease"] #[Model_10_neg_scaled, Model_10_scaled ]
ROIs = ["choice_mOFC_6mm_ts", "overlap_vmPFC_6mm_ts"]
goes = ["1", "2"]
conditions = ["Face", "Money"]
		...
		for condition in conditions:
			for model in models:
				for subnum in subnums:
					...
					tmp_job_file = tmp_job_file.replace( "SUB_ROI_VAR", str(ROI) )
					tmp_job_file = tmp_job_file.replace( "SUB_MODEL_VAR", str(model) )
					tmp_job_file = tmp_job_file.replace( "SUB_CONDITION_VAR", str(condition) )
			
			
					#make fname and write job file to cwd
					tmp_job_fname = "_".join( ["L2", "PPI", ROI, subnum, go] )
