...
template_f = file("flirt_mask2func.sh")  #job template location (on head node)
...
for subnum in subnums:
	...
	for run in runs:
		...
		tmp_job_file = tmp_job_file.replace( "SUB_SUBJ_VAR", str(subnum) )
		

		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["extractTS", subnum, run] )
