...
template_f = file("TEST.sh")  #job template location (on head node)
...
for subject in subjects:
	for run in runs:
		...
		runstr = "%02d" %(run)
		subjectstr = "%02d" %(subject)
		delayt = 5
		tmp_job_file = template.replace( "SUB_USEREMAIL_SUB", useremail )
		tmp_job_file = tmp_job_file.replace( "SUB_SUBNUM_SUB", str(subject) )
		tmp_job_file = tmp_job_file.replace("SUB_RUN_SUB",str(run) )
		tmp_job_file = tmp_job_file.replace("SUB_DELAY_SUB",str(delayt) )


		#make fname and write job file to cwd
		tmp_job_fname = "_".join( ["TEST", subjectstr, runstr] )
