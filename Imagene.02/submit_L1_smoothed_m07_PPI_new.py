...
template_f = file("L1_smoothed_m07_PPI_new.sh")  #job template location (on head node)
experiment = "Imagene.02"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 195         #leave one in q to keep them moving through
min_jobs = 95                 #minimum number of jobs to keep in q even when crowded
	...
	if os.path.isfile(fname):
		...
		for line in input_list:
			...
			for myline in mylines:
				...
				tmp_job_fname = "_".join( ["ppi", subnum, str(run)] )
