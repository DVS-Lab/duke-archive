...
template_f = file("fslprestats2_smooth_fix.sh")  #job template location (on head node)
...
tasks = [ "Risk", "MID", "Framing", "Resting" ] #task name
runs = range(1,4)
...
for task in tasks:
	for subnum in subnums:
		for run in runs:	
			...
			tmp_job_fname = "_".join( ["fsl", subnum, task, str(run), "6mm"] )
