...
template_f = file("L1_smoothed_m07.sh")  #job template location (on head node)
experiment = "Imagene.02"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 75         #leave one in q to keep them moving through
...
splits = [ '1' ] #going through all the subs. seems to be split at L2 and L3, so update those sublists tonight.

maindir = '/home/dvs3/linux/Imagene.02/StrParc'
...
os.chdir(tmp_dir)
for split in splits:
	fname = '%s/all_good_runs_n577.txt'  % ( maindir )
	if os.path.isfile(fname):
		...
		for line in input_list:
			...
			for myline in mylines:
				...
				tmp_job_fname = "_".join( ["mid03", subnum, str(run)] )
