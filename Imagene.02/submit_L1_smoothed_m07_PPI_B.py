...
template_f = file("L1_smoothed_m07_PPI_B.sh")  #job template location (on head node)
experiment = "Imagene.02"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 95         #leave one in q to keep them moving through
min_jobs = 50                 #minimum number of jobs to keep in q even when crowded
	...
	if os.path.isfile(fname):
		...
		for line in input_list:
			...
			for myline in mylines:
				...
				tmp_job_fname = "_".join( ["mid07", subnum, str(run)] )
