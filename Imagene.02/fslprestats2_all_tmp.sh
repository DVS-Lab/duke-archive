...
if [ ${SMOOTH} -eq 6 ]; then
	...
	DATA=${MYOUTDIR}/run${R}/swacrun${R}.nii.gz
fi

#rm -rf ${MYINDIR}/run${R}.nii
if [ ! -e ${MYINDIR}/swacrun${R}.nii ]; then
	echo "missing file: ${MYINDIR}/swacrun${R}.nii" >> $MAINDIR/__missingfiles_dirty.txt	
else

	# -- set output and remove existing --
	OUTPUT=${MYOUTDIR}/prestats${R}_${SMOOTH}mm
	rm -rf ${OUTPUT}.feat
	sleep 5s
	rm -rf ${OUTPUT}.feat
	sleep 5s
	...
	<$TEMPLATE> ${MYOUTDIR}/prestats${R}_${SMOOTH}mm.fsf
	feat ${MYOUTDIR}/prestats${R}_${SMOOTH}mm.fsf
	...
	if [ -e $GENDATA ]; then
		...
		echo "failed: $GENDATA" >> $MAINDIR/__faileddata_dirty.txt
	fi
