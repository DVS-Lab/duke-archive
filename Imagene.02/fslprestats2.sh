...
if [ ! -e ${MYINDIR}/swacrun${R}.nii ]; then
	...
	OUTPUT=${MYOUTDIR}/prestats${R}_0mm
	rm -rf ${OUTPUT}.feat
	...
	if [ -e $CONFOUNDEVSFILE ]; then
		...
		fslmaths ${MYOUTDIR}/run${R}/swacrun${R}.nii ${MYOUTDIR}/run${R}/swacrun${R}_scrubbed
	fi


	# -- run remaining preprocessing steps and ica --
	DATA=${MYOUTDIR}/run${R}/swacrun${R}_scrubbed.nii.gz
	TEMPLATE=${SCRIPTDIR}/hp_bet_melodic.fsf
	sed -e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@DATA@'$DATA'@g' \
	-e 's@NVOLUMES@'$NVOLUMES'@g' \
	<$TEMPLATE> ${MYOUTDIR}/prestats${R}_0mm.fsf
	feat ${MYOUTDIR}/prestats${R}_0mm.fsf
	...
	cd ${OUTPUT}.feat
	mkdir mc
	cp ${MYOUTDIR}/run${R}/rp_crun${R}.txt mc/prefiltered_func_data_mcf.par #motion parameters from SPM
	mkdir reg
	cp ${MYOUTDIR}/run${R}/wbmeancrun${R}.nii reg/example_func.nii.gz
	cp ${MYOUTDIR}/run${R}/wrender${S}_anat.nii reg/highres.nii.gz
