...
template_f = file("L1_smoothed_ppi.sh")  #job template location (on head node)
experiment = "Imagene.02"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 56         #leave one in q to keep them moving through
...
os.chdir(tmp_dir)
for split in splits:
	fname = '%s/list%s_runs_edit.txt'  % ( maindir, split )
	if os.path.isfile(fname):
		...
		for line in input_list:
			...
			for myline in mylines:
				...
				tmp_job_fname = "_".join( ["mid_ppi", subnum, str(run)] )
