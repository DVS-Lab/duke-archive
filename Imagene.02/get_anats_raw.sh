...
#SUBJECTS=( 10360 11067 )

#FNIRT will want a whole head and a BETed image. need to make _brain from wholehead
STANDARD=${FSLDIR}/data/standard/MNI152_T1_1mm_brain
LENGTH=${#SUBJECTS[@]}
let LENGTH=$LENGTH-1


OUTPUTDIR=$MAINDIR/for_CR_230_T1s
mkdir -p $OUTPUTDIR
...
for x in `seq 0 $LENGTH`; do
	...
	FLIRTIN=${EXPERIMENT}/Analysis/TaskData/${SUBJ}/${SUBJ}_anat
	FLIRTOUT=${OUTPUTDIR}/${SUBJ}_anat
	fslmaths $FLIRTIN $FLIRTOUT
