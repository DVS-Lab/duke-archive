...
if [ $SKIP -eq 1 ]; then
	...
	OUTDIR=${MAINDIR}/Logs/ica_may2011/fsl_motion_outlier_skips/GO_${GO}
else
...
# 		$FSLDIR/bin/fslmaths filtered_func_data.nii.gz -mas new_mask.nii.gz new_filtered_func_data
	...
	if [ -e ${OUTPUTREAL}/bad_timepoints.txt ]; then
		...
		OUTDIR=${MAINDIR}/Logs/ica_may2011/fsl_motion_outlier_SUCCESS/GO_${GO}
	else
		fsl_motion_outliers ${DATA} ${NDISDAQS} ${OUTPUTREAL}/bad_timepoints.txt
		if [ -e ${OUTPUTREAL}/bad_timepoints.txt ]; then
			OUTDIR=${MAINDIR}/Logs/ica_may2011/fsl_motion_outlier_SUCCESS/GO_${GO}
		else
			OUTDIR=${MAINDIR}/Logs/ica_may2011/fsl_motion_outlier_FAIL/GO_${GO}
		fi
	fi
fi
