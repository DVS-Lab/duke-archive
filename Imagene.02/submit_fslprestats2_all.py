...
template_f = file("fslprestats2_unsmoothed_clean.sh")  #job template location (on head node)
experiment = "Imagene.02"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 85         #leave one in q to keep them moving through
...
#tasks = [ "MID", "Risk", "Framing", "Resting" ] #task name
tasks = [ "Framing" ] #task name

#runs = range(1,4) #see below
#smooths = [ "0", "6" ]
smooths = [ "0" ]
...
daemonize('/dev/null',os.path.join(start_dir,'daemon_task.log'),os.path.join(start_dir,'daemon_task.log'))
sys.stdout.close()
os.chdir(start_dir)
temp=time.localtime()
...
log_name=os.path.join(start_dir,'daemon_task.log')
log=file(log_name,'w')
	...
	for task in tasks:
		for subnum in subnums:
			...
			for run in runs:	
				...
				tmp_job_file = tmp_job_file.replace( "SUB_TASK_SUB", task )
				...
				tmp_job_fname = "_".join( ["fsl", subnum, task, str(run), smooth] )
