...
template_f = file("L1_smoothed.sh")  #job template location (on head node)
experiment = "Imagene.02"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 45         #leave one in q to keep them moving through
	...
	if os.path.isfile(fname):
		...
		for line in input_list:
			...
			for myline in mylines:
				...
				tmp_job_fname = "_".join( ["mid", subnum, str(run)] )
