...
do 
  ...
  do
	...
	for COPE in 1 2 3 4
	do
		for ROI in lTPJ_charself_5mm midTG_charself_5mm lPCC_charself_5mm
		do

	    #directory for feat logs
	    CONFILEDIR=${EXPERIMENT}/Analysis/Framing/FSL/${SUBJ}/NoLapses_PPI/model10/${ROI}/${CON}.gfeat/report_log.html
	    COPEFILEDIR=${EXPERIMENT}/Analysis/Framing/FSL/${SUBJ}/NoLapses_PPI/model10/${ROI}/${CON}.gfeat/cope${COPE}.feat/report_log.html
	    LOGFILEDIR=${EXPERIMENT}/Analysis/Framing/Logs/NoLapses_PPI/L2_m10_go_1/${SUBJ}/ppi_L2_m_10_${SUBJ}_${CON}_${ROI}*
	 
	    echo "Sub $SUBJ, $ROI, $CON, $GO, $COPE" >> ${OUTDIR}/L2_PPI_LogCheck.txt

	    grep "ERROR" $CONFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "Error" $CONFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "Warning" $CONFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "ERROR" $COPEFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "Error" $COPEFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "Warning" $COPEFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "ERROR" $LOGFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "Error" $LOGFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
	    grep "Warning" $LOGFILEDIR >> ${OUTDIR}/L2_PPI_LogCheck.txt
