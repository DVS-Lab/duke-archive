...
MODEL="Choice"
OUTDIR=${EXPERIMENT}/Analysis/Framing/Logs/Model${MODEL}_NoLapses/OutputCheck
mkdir -p ${OUTDIR}
EXAMPLEFILEDIR=${EXPERIMENT}/Analysis/Framing/FSL/10156/NoLapses_RL/model10/run1.feat/report_log.html
...
#have 3rd run data missing so change run
...
FSLEVDIR=$EXPERIMENT/Analysis/Framing/EVfiles_Choice/Model${MODEL}_NoLapses/${SUBJ}

	    #directory for feat log
FEATFILEDIR=${EXPERIMENT}/Analysis/Framing/FSL/${SUBJ}/NoLapses_RL/model${MODEL}/run${run}.feat/report_log.html
LOGFILEDIR=${EXPERIMENT}/Analysis/Framing/Logs/Model${MODEL}_NoLapses/L1_m${MODEL}_go${GO}/${SUBJ}/L1m_${MODEL}_${SUBJ}_0${run}*.out
echo "Sub $SUBJ,  run$run, go$GO" >> ${OUTDIR}/L1.txt

if [ -e ${FSLEVDIR}/run${run}/${SUBJ}_misses_run${run}.txt ]; then
	echo "Has MISSES" >> ${OUTDIR}/L1.txt
	grep "Warning" $FEATFILEDIR >> ${OUTDIR}/L1.txt

else
	echo "Has no misses" >> ${OUTDIR}/L1.txt
	message=`grep "Warning" $FEATFILEDIR`
	if [[ $message == $NoMissWarn ]]; then
		echo "Warnings ALL CLEAR!" >> ${OUTDIR}/L1.txt
	else #error message not just missing EV
		grep "Warning" $FEATFILEDIR >> ${OUTDIR}/L1.txt
	fi
fi

	grep "Error" $FEATFILEDIR >> ${OUTDIR}/L1.txt


	grep "ERROR" $FEATFILEDIR >> ${OUTDIR}/L1.txt


	grep "Could not find" $FEATFILEDIR >> ${OUTDIR}/L1.txt
