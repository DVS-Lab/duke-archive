...
NEWANATDIR=${EXPERIMENT}/Analysis/TaskData/${SUBJ}_test
mkdir -p $NEWANATDIR
...
for RUNS in `seq $NRUNS`; do
	...
	if [ $RUNS -ge $MID ] && [ $RUNS -le $MID_STOP ]; then
		let R1=$R1+1
		MOUTDIR=${EXPERIMENT}/Analysis/TaskData/${SUBJ}_test/MID
		mkdir -p ${MOUTDIR}
		cd ${MAINFUNCDIR}/${SUBJ_FULL}/${RUN_PREFIX}_${RUN}_corrected

		rm -rf reoriented_run${RUNS}.bxh
		rm -rf reoriented_run${RUNS}.dat

		bxhreorient --orientation LAS ${RUN_PREFIX}_${RUN}_corrected.bxh reoriented_run${RUNS}.bxh
	...
	if [ $RUNS -ge $FRAMING ] && [ $RUNS -le $FRAMING_STOP ];then
		let R2=$R2+1
		FOUTDIR=${EXPERIMENT}/Analysis/TaskData/${SUBJ}_test/Framing
		mkdir -p ${FOUTDIR}
		cd ${MAINFUNCDIR}/${SUBJ_FULL}/${RUN_PREFIX}_${RUN}_corrected
		rm -rf reoriented_run${RUNS}.bxh
		rm -rf reoriented_run${RUNS}.dat
	...
	if [ $RUNS -ge $RISK ] && [ $RUNS -le $RISK_STOP ];then
		...
		ROUTDIR=${EXPERIMENT}/Analysis/TaskData/${SUBJ}_test/Risk
		mkdir -p ${ROUTDIR}
		cd ${MAINFUNCDIR}/${SUBJ_FULL}/${RUN_PREFIX}_${RUN}_corrected
		rm -rf reoriented_run${RUNS}.bxh
		rm -rf reoriented_run${RUNS}.dat
	...
	if [ $RUNS -eq $REST ];then
		let R4=$R4+1
		RESTOUTDIR=${EXPERIMENT}/Analysis/TaskData/${SUBJ}_test/Resting
		mkdir -p ${RESTOUTDIR}
		cd ${MAINFUNCDIR}/${SUBJ_FULL}/${RUN_PREFIX}_${RUN}_corrected
		rm -rf reoriented_run${RUNS}.bxh
		rm -rf reoriented_run${RUNS}.dat
