...
if [ $SKIP -eq 1 ]; then
	...
	if [ $FNIRT -eq 1 ]; then
		CHECKREG=${OUTPUT}.ica/reg_test${TESTN}/example_func2standard.png
		if [ -e $CHECKREG ]; then
			...
			REGDIR=${OUTPUT}.ica/reg_test${TESTN}
			rm -rf $REGDIR
			mkdir -p $REGDIR
			cd $REGDIR
			...
			$FSLDIR/bin/fslmaths standard -bin -dilF standard_mask -odt char
			$FSLDIR/bin/fslmaths $ANATH highres_head
			$FSLDIR/bin/fslroi $DATA example_func 0 1
			...
			$FSLDIR/bin/fnirt --in=highres_head --aff=highres2standard.mat --cout=highres2standard_warp --iout=highres2standard --jout=highres2standard_jac --config=T1_2_MNI152_2mm --ref=standard_head --refmask=standard_mask --warpres=10,10,10 --subsamp=4,4,2,2,1,1 --applyrefmask=1,1,1,1,1,1
