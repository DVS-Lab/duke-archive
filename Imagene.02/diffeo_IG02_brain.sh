...
OUTPUTDIR=$TMPDIR/T1s_2mm_brain_ANTS
cp -r $MAINDIR/T1s_2mm_brain $TMPDIR/T1s_2mm_brain_ANTS
...
for x in `seq 0 $LENGTH`; do
	...
	FLIRTIN=${EXPERIMENT}/Analysis/TaskData/${SUBJ}/${SUBJ}_anat_brain.nii.gz
	FLIRTOUT=${OUTPUTDIR}/${SUBJ}_${FLIRTOUTEXT}_${TYPE}.nii.gz
	OUTMAT=${OUTPUTDIR}/${SUBJ}_${FLIRTOUTEXT}_${TYPE}.mat
...
#save the transforms for the whole head model (will make in other script)
mkdir -p $MAINDIR/T1s_meanT1target_2mm_wholehead
mv ${OUTPUTDIR}/*.mat $MAINDIR/T1s_meanT1target_2mm_wholehead/.
