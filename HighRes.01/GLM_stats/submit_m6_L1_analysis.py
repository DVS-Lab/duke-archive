...
template_f = file("m6_L1_analysis.sh")  #job template location (on head node)
experiment = "HighRes.01"    #experiment name for qsub
nodes = 400                   #number of nodes on cluster
maintain_n_jobs = 20         #leave one in q to keep them moving through
min_jobs = 20                 #minimum number of jobs to keep in q even when crowded
n_fake_jobs = 1               #during business hours, pretend there are extra jobs to try and leave a few spots open
...
max_run_hours = 900000	#maximum number of hours submission script can run
warning_time = 900000         #send out a warning after this many hours informing you that the deamon is still running
...
#subnums = [ "1002", "1005", "1006", "1007", "1008", "1009", "1010", "1011", "1012", "1013", "1014", "1015", "1016", "1017", "1018", "1019","1020", "1021", "1022", "1023" ]
subnums = [ "1002", "1005", "1006", "1007", "1008", "1009" ]

trials = range(1,57)
runs = range(1,6)  #range cuts the last number off. any single runs should still be in [ ]
goes = [ "4" ]
smooths = [ "0" ]
fnirts = [ "1" ]
setorigins = [ "1" ]
...
#see how many jobs we have  in
	...
	for line in status_list:
		...
		#if job is mine
		if (line.find(username) > 0) & (line.find("interact.q") < 0) & (line.find("HiRes") > 0):   #name is in the line, not including first spot
			n_jobs = n_jobs + 1
				...
				for subnum in subnums:
					for run in runs:
						for trial in trials:
							...
							tmp_job_file = tmp_job_file.replace( "SUB_FNIRT_SUB", str(fnirt) )
							tmp_job_file = tmp_job_file.replace( "SUB_SETORIGIN_SUB", str(setorigin) )
							tmp_job_file = tmp_job_file.replace( "SUB_TRIAL_SUB", str(trial) )
							
							#make fname and write job file to cwd
							tmp_job_fname = "_".join( ["HiRes_6trial", trialstr, subnum, runstr, go] )
							...
							while n_jobs >= maintain_n_jobs: 
								...
								if n_jobs >= maintain_n_jobs: 
									...
									cmd = "qsub -l h_vmem=12G -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
