...
nodes = 16                   #number of nodes on cluster
maintain_n_jobs = 16         #leave one in q to keep them moving through
...
max_run_hours = 72	#maximum number of hours submission script can run
...
subnums = ["10001", "10002", "10003", "10004"]

#subnums = ["47731"]

#should be entered in quotes to be used as strings  
runs = range(1,6)  #[ 1, 2, 3, 4, 5, 6, 7, 8 ] range cuts the last number off any single runs should still be in [ ]
goes = ["1", "2"]
smooths = ["4", "0"]
fnirts = ["0"] #add 1 later
		...
		for smooth in smooths:
			for subnum in subnums:
				...
				for run in runs:
					...
					tmp_job_fname = "_".join( ["HiRes_L1", subnum, runstr, smooth, fnirt, go] )
