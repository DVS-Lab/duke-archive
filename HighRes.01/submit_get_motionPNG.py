...
template_f = file("get_motionPNG.sh")  #job template location (on head node)
...
for go in goes:
	for subnum in subnums:
		for run in runs:
			...
			tmp_job_fname = "_".join( ["motion", subnum, runstr, go] )
			...
			while n_jobs >= maintain_n_jobs: 
				...
				if n_jobs >= maintain_n_jobs: 
					...
					cmd = "qsub -l h_vmem=6G -v EXPERIMENT=%s %s"  % ( experiment, tmp_job_fname )
